...
def register(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			user = Account.objects.create_user(nom=nom, prenom=prenom, email=email, username=username, password=password)
			...
			mail_subject = "S'il vous plaît activer votre compte"
			...
			# messages.success(request, " Merci de vous êtes enrégistré chez nous. Nous nous avons envoyé un mail de vérification à votre addresse mail. S'il vous plaît vérifier. ")
...
def activate(request, uidb64, token):
	...
	if user is not None and default_token_generator.check_token(user, token):
		...
		messages.error(request, "Lien d'action invalide")
...
def forgotPassword(request):
	if request.method == 'POST':
		...
		if Account.objects.filter(email=email).exists():
			...
			messages.success(request, " Nous vous avons envoyez un mail pour Rénitialiser votre mot de passe. ")
			return redirect('login')
		else:
...
def resetPassword(request):
	if request.method == 'POST':
		...
		if password == confirm_password:
			...
			user.save()
			messages.success(request, "Reinitialisation du mot de passe Réussir")
			return redirect('login')
		else:
...
def edit_profile(request):
	...
	if request.method == 'POST':
		...
		if user_form.is_valid() and profile_form.is_valid():
			user_form.save()
			profile_form.save()
			messages.success(request, 'Votre profile est bien mis à jours.')
